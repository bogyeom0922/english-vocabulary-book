<?php
    ...
    if ( strcasecmp($row['Word'], $Word) == 0 ) {
        ...
        <?php
        if ($_SESSION['i'] < 50) { 
            echo "<script>location.href='easygame.php'</script>";
        }
        else if ($_SESSION['i'] >= 50) { 
            echo "<script>location.href='EasyRanking.php'</script>";
        }
    } else { ?>
        <script>
    ...
    mysqli_query($connect, "UPDATE user SET Stage = 'Easy' WHERE Use_id = '$Use_id'");
