<?php
    ...
    if ( strcasecmp($row['Word'], $Word) == 0 ) {
        ...
        <?php
        if ($_SESSION['i'] < 100) { 
            echo "<script>location.href='normalgame.php'</script>";
        }
        else if ($_SESSION['i'] >= 100) { 
            echo "<script>location.href='NormalRanking.php'</script>";
        }
    } else { ?>
        <script>
    ...
    mysqli_query($connect, "UPDATE user SET Stage = 'Normal' WHERE Use_id = '$Use_id'");
