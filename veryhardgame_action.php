<?php
    ...
    if ( strcasecmp($row['Word'], $Word) == 0 ) {
        ...
        <?php
        if ($_SESSION['i'] < 300) { 
            echo "<script>location.href='verygame.php'</script>";
        }
        else if ($_SESSION['i'] >= 300) { 
            echo "<script>location.href='VeryHardRanking.php'</script>";
        }
    } else { ?>
        <script>
    ...
    mysqli_query($connect, "UPDATE user SET Stage = 'VeryHard' WHERE Use_id = '$Use_id'");
